...
function pagemanager_toolbar($image_ext, $save_js) 
{
    ...
    foreach ($toolbar as $tool) 
	{
		...
		$res.= tag('img class="' . $class . '" src="' . $img . '"' . ($tool != 'help' ? ' onclick="pagemanager_do(\'' . $tool . '\'); return false;" alt="' . $tx['pagemanager']['op_' . $tool] . '"' : ' alt="' . $tx['pagemanager']['op_' . $tool] . '"')) . '<span>' . $tx['pagemanager']['op_' . $tool] . '</span>' . "\n";
